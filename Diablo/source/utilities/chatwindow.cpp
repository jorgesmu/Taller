...
const int CLOSE_BUTTON_SIDE = 10;
...
// Metodos
void ChatWindow::init(ResMan* rm, int font_size, int height, int width, const SDL_Color& color) {
	// Ctor
...
int ChatWindow::handleInput(SDL_Event &event) {
	// Falta ver lo del boton de exit
...
void ChatWindow::show(SDL_Surface* dest, int x, int y) {
...
void ChatWindow::addMsg(const std::string& nick, const std::string& msg) {
	...
	if(msg_queue.size() > this->max_msg_count) {
		msg_queue.pop_front();
	}
}
